...
func loadProxies(filePath string) {
	file, err := os.Open(filePath)
	if err != nil {
		log.Fatalf("打开代理文件失败: %v", err)
	}
	defer file.Close()
	...
	if len(proxies) == 0 {
		log.Fatal("代理文件为空或格式错误")
	}
	log.Printf("已加载 %d 个代理", len(proxies))
}
...
func sendTelegramNotification(targetURL, username, password string, v2rayConfigs []string) error {
	...
	proxyURLParsed, err := url.Parse(currentProxy)
	if err != nil {
